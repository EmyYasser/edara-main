...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await query("insert into product set ? ", movie);
      res.status(200).json({
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const movie = await query("select * from product where id = ?", [
        req.params.id,
      ]);
      if (!movie[0]) {
      ...
      // 4- UPDATE MOVIE
      await query("update product set ? where id = ?", [movieObj, movie[0].id]);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      fs.unlinkSync("./upload/" + movie[0].image_url); // delete old image
      await query("delete from product where id = ?", [movie[0].id]);
      res.status(200).json({
...
router.get("", async (req, res) => {
  ...
  const movies = await query(`select * from product ${search}`);
  movies.map((movie) => {
